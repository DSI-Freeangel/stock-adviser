...
public class FinancialDataProviderImplTest {
    ...
    public void getFinancialDataWhenDataActualInDB() {
        givenFinancialDataPresentInDB(STOCK_CODE_FULL, LocalDate.now().minusDays(3));
    ...
    public void getFinancialDataWhenNoDataInDB() {
        givenFinancialDataIsNotPresentInDB(STOCK_CODE_FULL);
    ...
    public void getFinancialDataWhenDataIsNotActualInDB() {
        givenFinancialDataPresentInDB(STOCK_CODE_FULL, LocalDate.now().minusDays(31));
        givenDataSourceIsAbleToReturnFinancialData();
        Mono<StockOverviewData> financialData = financialDataProvider.getFinancialData(STOCK_CODE_FULL);
    ...
    private void givenFinancialDataPresentInDB(String stockCodeFull, LocalDate date) {
        when(financialDataRepository.findOneByStockCodeFullEquals(eq(stockCodeFull))).thenReturn(Mono.just(getFinancialDataEntity(stockCodeFull, date)));
    }

    private void givenFinancialDataIsNotPresentInDB(String stockCodeFull) {
        when(financialDataRepository.findOneByStockCodeFullEquals(eq(stockCodeFull))).thenReturn(Mono.empty());
    }

    private void givenDataSourceIsAbleToReturnEmptyFinancialData() {
        when(dataSource.getFinancialData(eq(STOCK_CODE_FULL))).thenReturn(Mono.empty());
    }

    private void givenDataSourceIsAbleToReturnFinancialData() {
        when(dataSource.getFinancialData(eq(STOCK_CODE_FULL))).thenReturn(Mono.just(getFinancialDataItem()));
    }
    ...
    private FinancialDataEntity getFinancialDataEntity(String stockCodeFull, LocalDate date) {
